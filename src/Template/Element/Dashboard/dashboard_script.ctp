	          		...
	          		scales: {
	              		yAxes: [{
	                  		ticks: {
	                      		display: false,
	                      		min: 0,
	                      		stepSize: <?php if (max($totalVisitors6Month) == 0) echo '80'; else { $maxAxis = round((max($totalVisitors6Month) + 200) / 10); echo $maxAxis; }  ?>,
	                      		max: <?php if (max($totalVisitors6Month) == 0) echo '80'; else { $maxAxis = max($totalVisitors6Month) + 200; echo $maxAxis; }  ?>
	                  	},
	                  	gridLines: {
	                    	drawBorder: false,
